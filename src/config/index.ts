...
const config = {
  ...
  minio: {
    ...
    presignedExpirySeconds: toNumber(process.env.MINIO_PRESIGNED_EXPIRY_SECONDS, 3600),
  },
  ...
  upload: {
    maxFileSize: toNumber(process.env.MAX_FILE_SIZE, 104857600),
  },

  processing: {
    thumbnail: {
      width: toNumber(process.env.THUMBNAIL_WIDTH, 400),
      quality: toNumber(process.env.THUMBNAIL_QUALITY, 80),
    },
    video: {
      ...
      resolutions: (process.env.VIDEO_RESOLUTIONS || '1080,720,480')
        .split(',')
        .map((resolution) => toNumber(resolution, 0))
